            ...
            <#else>
              <el-form-item label="${column.comment!}" prop="${column.attrName}">
                <el-select v-model="dataForm.${column.attrName}" placeholder="请选择">
                  <el-option label="WRATH" value="0"></el-option>
                </el-select>
              </el-form-item>
            </#if>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取信息
      getInfo () {
        this.$http.get('/${backendUrl}/${classname}/' + this.dataForm.${pk.attrName}).then(({ data: res }) => {
          if (res.code !== 0) {
      ...
      dataFormSubmitHandle: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          ...
          this.$http[!this.dataForm.${pk.attrName} ? 'post' : 'put']('/${backendUrl}/${classname}/', this.dataForm).then(({ data: res }) => {
            if (res.code !== 0) {
              return this.$message.error(res.msg)
            }
            this.$message({
