...
import personal.wrath.commons.tools.utils.MessageUtils;
import personal.wrath.commons.tools.utils.Result;
import personal.wrath.dto.TaskDTO;
...
public class ActTaskService extends BaseServiceImpl {
    ...
    public PageData<TaskDTO> page(Map<String, Object> params) {
        ...
        if(StringUtils.isNotEmpty((String)params.get("isRoleGroup"))&&"1".equals(params.get("isRoleGroup"))){
            ...
            for(Long role : roleResult.getData()){
    ...
    public void claimTask(String taskId) {
        ...
        if(StringUtils.isNotEmpty(task.getAssignee())){
            throw new RenException(ModuleErrorCode.TASK_CLIME_FAIL);
        }
    ...
    public void unclaimTask(String taskId) {
        ...
        if(lstGroupId.isEmpty()){
            throw new RenException(ModuleErrorCode.UNCLAIM_ERROR_MESSAGE);
        }
    ...
    public void doBackPreviousTask(String taskId, String comment) {
        ...
        if(this.isMultiInstance(taskId)){
            throw new RenException(ModuleErrorCode.BACK_PROCESS_PARALLEL_ERROR);
        }
        ...
        if(canBackActivitys.isEmpty()) {
            throw new RenException(ModuleErrorCode.SUPERIOR_NOT_EXIST);
        }
        ...
        for (Task task : tasks) {
            String commentMode = MessageUtils.getMessage(ModuleErrorCode.ROLLBACK_MESSAGE);
            if(StringUtils.isNotEmpty(comment)){
                commentMode += "[" +comment+"]";
            }
        ...
        for(PvmTransition pvmTransition : incomingTransitions){
            ...
            if ("parallelGateway".equals(type)){
                ...
                if(activityImpl.getOutgoingTransitions().size() > 1){
                    throw new RenException(ModuleErrorCode.BACK_PROCESS_HANDLEING_ERROR);
                }
    ...
    public void endProcess(String taskId, String comment) {
        if(isMultiInstance(taskId)){
            throw new RenException(ModuleErrorCode.END_PROCESS_PARALLEL_ERROR);
        }
        Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
        ...
        if(tasks.size() > 1){
            throw new RenException(ModuleErrorCode.END_PROCESS_HANDLEING_ERROR);
        }
        ...
        if (StringUtils.isEmpty(endActivity.getId())) {
            ...
            String message =MessageUtils.getMessage(ModuleErrorCode.END_PROCESS_MESSAGE);
    ...
    private ActivityImpl findActivitiImpl(String taskId, String activityId) {
        ...
        if (processDefinition == null) {
            throw new RenException(ModuleErrorCode.NONE_EXIST_PROCESS);
        }
    ...
    public void backToFirst(String taskId, String comment) {
        if(this.isMultiInstance(taskId)){
            throw new RenException(ModuleErrorCode.REJECT_PROCESS_PARALLEL_ERROR);
        }
        Map variables = null;
        HistoricTaskInstance currTask = historyService.createHistoricTaskInstanceQuery().taskId(taskId).singleResult();
        ProcessInstance processInstance = runtimeService.createProcessInstanceQuery().processInstanceId(currTask.getProcessInstanceId()).singleResult();
        ...
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(currTask.getProcessInstanceId())
                .taskDefinitionKey(currTask.getTaskDefinitionKey()).list();
        if(tasks.size() > 1){
            throw new RenException(ModuleErrorCode.REJECT_PROCESS_HANDLEING_ERROR);
        }
        ActivityImpl currActivity = processDefinitionEntity.findActivity(currTask.getTaskDefinitionKey());
        ...
        variables = processInstance.getProcessVariables();
        for (Task task : tasks) {
            String commentMode = MessageUtils.getMessage(ModuleErrorCode.REJECT_MESSAGE);
            if(StringUtils.isNotEmpty(comment)){
                commentMode += "[" + comment+"]";
            }
            taskService.addComment(task.getId(), task.getProcessInstanceId(), commentMode);
